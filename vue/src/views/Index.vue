<template>
  <div>
    ...
    <p>Test Join Room</p>
    <button v-for="e in $store.state.data.game" v-bind:key="e.id" @click="$router.push(`/game/${e.id}`)" class="bg-green-500 m-1">
      {{e.id}}
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async createGame(){
        let res = await this.$store.dispatch("entitySubmit", {type: "game", body: {}})
        console.log(res)
